          ...
          <div className="mt-10 mx-auto max-w-xl">
                          ...
                          <FormControl>
                            <div className="relative">
                              ...
                              <Input
                                ...
                                autoComplete="url"
                                disabled={loading}
                                className="h-11 pl-9 bg-white/[0.03] border-white/10 text-white placeholder:text-white/30 focus-visible:border-[#55E039]/40 focus-visible:ring-[#55E039]/20"
                                {...field}
                              />
                            </div>
                          </FormControl>
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-3 text-center">
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                ...
                <p className="mt-1 text-xs text-white/55">FDA/FTC enforcement rules</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <p className="text-2xl font-extrabold text-[#55E039]">~30s</p>
                <p className="mt-1 text-xs text-white/55">Average scan time</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                <p className="text-2xl font-extrabold text-[#55E039]">$0</p>
                <p className="mt-1 text-xs text-white/55">No card, no commitment</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="mt-10 space-y-8">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 backdrop-blur-xl">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div>
                  <p className="text-xs font-bold text-white/50 uppercase tracking-[0.2em] mb-2">Audit complete</p>
                  <p className="text-sm text-white/60 break-all">{result.website_url}</p>
                  {result.page_title && (
                    <p className="mt-1 text-sm text-white font-semibold">{result.page_title}</p>
                  )}
                  {result.summary && (
                    <p className="mt-3 text-sm text-white/70 max-w-xl leading-relaxed">{result.summary}</p>
                  )}
                </div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-4">
                <Stat label="Total flags" value={result.flag_count} icon={<AlertTriangle className="h-4 w-4 text-white/50" />} />
            ...
            <button
              ...
              className="block mx-auto text-sm text-white/50 hover:text-white transition-colors"
            >
              Scan another URL
            </button>
          </div>
...
function FlagCard({ flag, index }: { flag: PublicFlag; index: number }) {
  ...
  return (
    <div className={`rounded-xl border p-5 ${RISK_BG[flag.risk_level]}`}>
      ...
      {flag.matched_text && (
        <div className="mb-3">
          <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-1">Matched phrase</p>
          <p className="text-sm font-semibold text-white">&ldquo;{flag.matched_text}&rdquo;</p>
        </div>
      )}
      {flag.context && flag.context !== flag.matched_text && (
        <div className="mb-3">
          <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-1">In context</p>
          <p className="text-sm text-white/70 italic leading-relaxed">{flag.context}</p>
        </div>
      )}
      {flag.reason && (
        <div className="mb-3">
          <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-1">Why it&apos;s flagged</p>
          <p className="text-sm text-white/80 leading-relaxed">{flag.reason}</p>
        </div>
      )}
      {flag.alternative && (
